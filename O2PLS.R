...
# Cross-validation
# We first want to find the best number of components to use
# The best number is based on MSE value for each combination, it does not take in consideration overfitting
# So crossvalidation has to be completed by permutation testing to be sure the suggest combination does not overfit

a <- c(1,2,3,4,5)
ax <- c(1,2,3,4,5)
ay <- c(1,2,3,4,5)

results <- crossval_o2m(features_df1, features_df2, p_thresh = 4000, q_thresh = 4000, a, a, a, nr_folds = 6)
print(results)
...
write_xlsx(loadings_list_X_df, sprintf("O2PLS_DA/%s/data/loadings_X_df_%s_%s_%s.xlsx", folder, joint, ortho_x, ortho_y))
write_xlsx(loadings_list_Y_df, sprintf("O2PLS_DA/%s/data/loadings_Y_df_%s_%s_%s.xlsx", folder, joint, ortho_x, ortho_y))
...
write_xlsx(score_X_df, sprintf("O2PLS_DA/%s/data/score_X_df_%s_%s_%s.xlsx", folder, joint, ortho_x, ortho_y))
write_xlsx(score_Y_df, sprintf("O2PLS_DA/%s/data/score_Y_df_%s_%s_%s.xlsx", folder, joint, ortho_x, ortho_y))
